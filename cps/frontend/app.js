...
function App() {
  ...
  const apiKey = prompt('Enter ADMIN API KEY for demo UI:');
  ...
  async function createDb() {
    ...
    const res = await fetch('/api/databases', { method: 'POST', headers: { 'content-type': 'application/json', 'x-api-key': apiKey }, body: JSON.stringify(body) });
  ...
  async function provision(dbId) {
    ...
    const res = await fetch(`/api/databases/${dbId}/provision-user`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-api-key': apiKey }, body: JSON.stringify(body) });
    if (res.ok) {
