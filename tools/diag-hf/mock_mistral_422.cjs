...
// Also accept the /models/:owner/:model/chat path which proxy may call
app.post('/models/:owner/:model/chat', (req, res) => {
  const body = req.body || {};
  if (body.parameters) {
    return res.status(422).json({ object: 'error', message: { detail: [{ type: 'extra_forbidden', loc: ['body','parameters'], msg: 'Extra inputs are not permitted', input: body.parameters }] }, type: 'invalid_request_error' });
  }
  // If inputs is an array (our router sends inputs: messages)
  if (body.inputs) {
    const flat = Array.isArray(body.inputs) ? body.inputs.map(m => (m && m.content) ? m.content : String(m || '')).join('\n') : String(body.inputs || '');
    return res.json({ object: 'response', output: [{ generated_text: 'ok chat: ' + String(flat).slice(0,120) }] });
  }
  if (body.input) {
    return res.json({ object: 'response', output: [{ generated_text: 'fallback OK: ' + String(body.input).slice(0,120) }] });
  }
  return res.json({ object: 'response', output: [{ generated_text: 'ok default chat' }] });
});
