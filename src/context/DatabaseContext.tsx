  ...
  const exportSchema = useCallback((format: string) => {
    ...
    switch (format.toLowerCase()) {
      ...
      case 'postgresql':
        ...
        break;
      case 'sqlserver':
        script = generateSQLServerScript(tables, relationships, indexes, constraints, users, permissions, currentSchema.name);
        break;
      case 'oracle':
...
function generateSQLServerScript(tables: Table[], relationships: Relationship[], _indexes: Index[], _constraints: Constraint[], _users: User[], _permissions: Permission[], schemaName?: string): string {
  const dbName = schemaName && schemaName.trim() ? sanitizeDbName(schemaName) : `EXPORTED_DB_${new Date().toISOString().slice(0,10).replace(/-/g,'')}_${new Date().toTimeString().slice(0,8).replace(/:/g,'')}`;
  ...
  tables.forEach(table => {
    ...
    const columnDefs = table.columns.map(col => {
      ...
      if (col.identity) {
        ...
        const inc = col.identity.increment ?? 1;
        def += ` IDENTITY(${seed},${inc})`;
      }
      if (!col.nullable) def += ' NOT NULL';
      const formattedDefault = formatDefaultValueForSQL(col.defaultValue);
      if (formattedDefault !== undefined) def += ` DEFAULT ${formattedDefault}`;
...
function generateOracleScript(tables: Table[], relationships: Relationship[], _indexes: Index[], _constraints: Constraint[], _users: User[], _permissions: Permission[]): string {
  ...
  tables.forEach(table => {
    ...
    const columnDefs = table.columns.map(col => {
      let def = `  ${col.name} ${col.type}`;
      // Oracle: simple identity support via GENERATED AS IDENTITY
      if (col.identity) {
        def += ' GENERATED BY DEFAULT AS IDENTITY';
      }
      if (!col.nullable) def += ' NOT NULL';
