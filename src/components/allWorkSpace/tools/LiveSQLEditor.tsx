...
`);
  ...
  // Track per-result dialect selection (index -> 'raw'|'mysql'|'oracle')
  const [resultDialects, setResultDialects] = useState<Record<number, 'raw' | 'mysql' | 'oracle'>>({});
  ...
  // Simple dialect formatter for display only. These are best-effort
  // transforms to make statements look like MySQL/Oracle style for preview.
  const formatForDialect = (statement: string | undefined, dialect: 'raw' | 'mysql' | 'oracle') => {
    if (!statement) return '';
    const s = statement.trim();
    if (dialect === 'raw') return s;

    // Uppercase common keywords for readability
    const keywords = ['select','from','where','create table','alter table','drop table','insert into','values','update','set','delete from','primary key','foreign key','references','default'];
    let transformed = s.replace(/\s+/g, ' ');
    for (const k of keywords) {
      const re = new RegExp(`\\b${k}\\b`, 'ig');
      transformed = transformed.replace(re, k.toUpperCase());
    }

    if (dialect === 'mysql') {
      // use backticks for identifiers and AUTO_INCREMENT hint
      transformed = transformed.replace(/"([A-Za-z0-9_]+)"/g, '`$1`');
      transformed = transformed.replace(/\bIDENTITY\s*\(([^)]+)\)/ig, 'AUTO_INCREMENT');
      transformed = transformed.replace(/\bCURRENT_TIMESTAMP\b/ig, 'CURRENT_TIMESTAMP');
      // ensure table identifiers after CREATE TABLE are quoted with backticks
  transformed = transformed.replace(/CREATE TABLE\s+"?([A-Za-z0-9_]+)"?/i, (_m: any, name: string) => `CREATE TABLE \`${name}\``);
      // after building the string, convert the escaped sequence \` to actual backtick
      transformed = transformed.replace(/\\`/g, '`');
    }

    if (dialect === 'oracle') {
      // use double quotes for identifiers, use GENERATED AS IDENTITY for identity
      transformed = transformed.replace(/`([A-Za-z0-9_]+)`/g, '"$1"');
      transformed = transformed.replace(/\bAUTO_INCREMENT\b/ig, 'GENERATED BY DEFAULT AS IDENTITY');
      transformed = transformed.replace(/\bCURRENT_TIMESTAMP\b/ig, 'SYSTIMESTAMP');
  transformed = transformed.replace(/CREATE TABLE\s+`?([A-Za-z0-9_]+)`?/i, (_m: any, name: string) => `CREATE TABLE "${name}"`);
    }
            ...
            <div className="space-y-4">
              {executionResults.map((result, index) => (
                <div key={index} className={`p-3 rounded-lg border ${result.success ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
                  ...
                  {/* Statement viewer: show the executed statement with dialect toggles */}
                  {result.statement && (
                    <div className="mb-3">
                      <div className="flex items-center justify-between mb-2">
                        <div className="text-sm font-medium">Executed Statement</div>
                        <div className="flex items-center gap-2">
                          {(['raw','mysql','oracle'] as const).map((d) => (
                            <button
                              key={d}
                              onClick={() => setResultDialects(prev => ({ ...prev, [index]: d }))}
                              className={`text-xs px-2 py-1 rounded ${resultDialects[index] === d ? 'bg-gray-200 dark:bg-gray-700' : 'bg-transparent'}`}
                            >
                              {d.toUpperCase()}
                            </button>
                          ))}
                      ...
                      <div className="bg-white dark:bg-gray-800 border rounded p-2 text-sm font-mono overflow-auto mb-2">
                        {formatForDialect(result.statement, resultDialects[index] || 'raw')}
                      </div>
                    </div>
                  )}
